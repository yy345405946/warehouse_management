...
public interface RukuOrderRepository extends JpaRepository<RukuOrder, Integer> {
    ...
    @Query(value = "select t.* from (select CONCAT(use_type,' ',(select c.name from category c where c.id = category) as category_str,' ',vendor,' ',name,' ', unit,' ',number,' ',ruku_date,' ',cost,' ',if(debate is null, '',debate),' ',if(checkout_date is null,'',checkout_date),' ',if(memo is null,'',memo),' ') as keyWords, " +
            "o.id, o.use_type, o.category, (select c.name from category c where c.id = o.category) as category_str, o.vendor, o.name, o.unit, o.number, o.ruku_date, o.cost, o.debate, o.checkout_date, o.memo, o.created_by, o.created_date, o.updated_by, o.updated_date from ruku_order o) t where t.keyWords like %:keyWords%", nativeQuery = true)
    List<RukuOrder> findByKeyWords(@Param(value = "keyWords") String keyWords);

    @Query(value = "select * from (select CONCAT(use_type,' ',(select c.name from category c where c.id = category) as category_str,' ',vendor,' ',name,' ', unit,' ',number,' ',ruku_date,' ',cost,' ',if(debate is null, '',debate),' ',if(checkout_date is null,'',checkout_date),' ',if(memo is null,'',memo),' ') as keyWords, " +
            "o.id, o.use_type, o.category, (select c.name from category c where c.id = o.category) as category_str, o.vendor, o.name, o.unit, o.number, o.ruku_date, o.cost, o.debate, o.checkout_date, o.memo, o.created_by, o.created_date, o.updated_by, o.updated_date from ruku_order o) t where ruku_date >= :startDate and ruku_date <= :endDate", nativeQuery = true)
    List<RukuOrder> findByDate(@Param(value = "startDate") String startDate, @Param(value = "endDate") String endDate);

    @Query(value = "select * from (select CONCAT(use_type,' ',(select c.name from category c where c.id = category) as category_str,' ',vendor,' ',name,' ', unit,' ',number,' ',ruku_date,' ',cost,' ',if(debate is null, '',debate),' ',if(checkout_date is null,'',checkout_date),' ',if(memo is null,'',memo),' ') as keyWords, " +
            "o.id, o.use_type, o.category, (select c.name from category c where c.id = o.category) as category_str, o.vendor, o.name, o.unit, o.number, o.ruku_date, o.cost, o.debate, o.checkout_date, o.memo, o.created_by, o.created_date, o.updated_by, o.updated_date from ruku_order o) t where keyWords like %:keyWords% and ruku_date >= :startDate and ruku_date <= :endDate", nativeQuery = true)
